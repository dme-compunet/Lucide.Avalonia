...
{
    public static string GenerateFromFile(string path, bool optimize = true)
    {
        var data = GenerateFromFile(path);

        return optimize ? GeometryPathOptimizer.Optimize(data) : data;
    }

    private static string GenerateFromFile(string path)
    {
